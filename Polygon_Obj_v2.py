...
#    def SIFT_adjacent(self, nID, nPT):
#        
#        if not self.KDlist[nPT]: 
#            print 'WARNING: None in kds list'
#            return
#        else: 
#            print 'Running SIFT_adjacent'
#            
#        px,py = self.KDlist[nPT].x, self.KDlist[nPT].y
#        dx,dy = SIFT.matchpoint( self.IDfname[nID], self.KDs[nPT] )
#        print px, py, dx, dy
#        self.alter_pt(self.IDs[nID], nPT, (px+dx, py+dy))
...
ones( (3,4,5) )  # A 3-D ARRAY OF 1.0
for i, value in enumerate( LIST OR TUPLE )  # CREATES AN INDEX AND VALUE FROM LIST
for this, that in zip( list1, list2 )  # LOOP THROUGH TWO OR MORE LISTS AT SAME TIME
...
matplotlib.mlab.csv2rec()   # HAVEN'T TRIED THIS YET. RETURNS NUMPY ARRAY
scipy.io.loadmat()  # LOADS MATLAB DATA FILES

>>INDEXING
mask = where( logical_and(t>10, t10 & t<12 RETURN 1.0, OTHERWISE 0.0 ) )

>>PLOTTING
USE matplotlib.pyplot.quiver() AND quiverkey() TO PLOT 2D VECTOR FIELD ARROWS
USE matplotlib.pyplot.arrow() TO PLOT A STICKPLOT
plt.imshow(image, origin='lower', interpolation='quadric')
'''
###################################################################
